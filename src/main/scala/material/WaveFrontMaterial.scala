...
object WaveFrontMaterial {
  ...
  def sampleImage(u: Double, v: Double, texture: BufferedImage, width: Int, height: Int): RGB = {
    val color: Int = texture.getRGB(Math.round(u * width).toInt, Math.round(v * height).toInt)
    //linear filtering
    val x = Math.floor(u * width).toInt
    val X = (x + 1) % width
    val y = Math.floor(u * width).toInt
    val Y = (y + 1) % width
    val Seq(f00, f10, f01, f11) =
      Seq((x, y), (X, y), (x, Y), (X, Y)).map(Function.tupled(texture.getRGB)).map(RGB.apply)
  ...
  def fromFile(lines: String): Seq[WaveFrontMaterial] = {
    val materials: Array[String] = lines
      .substring(lines.indexOf("newmtl"))
      .split("newmtl")
      .filter(!_.isEmpty)
    for {
      material <- materials
  ...
  def readImageFile(array: Array[String]): Option[BufferedImage] = array.headOption.flatMap {
    filename =>
