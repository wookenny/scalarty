...
case class CheckerValue(stepSize: Double,
                        ...
                        value2 : Option[Double] = None) extends ValueNode{
  ...
  override def value(position: Vector3): Double = {
    val pos: Vector3 = (position - offset.getOrElse(Vector3.ZERO)) mult  scaling.getOrElse(Vector3.ONE)
...
case class NoiseValue(seed: Long,
                      size: Double) extends ValueNode{

  val noise = new OpenSimplex(seed, size)

  override def value(position: Vector3): Double = noise.evalNormalized(position.x, position.y, position.z)
}

case class MultilayerNoiseValue(seed: Long,
                                size: Double,
                                octaves: Option[Int]) extends ValueNode{
...
// map single value
case class ClampValue(node: ValueNode,
                      minValue: Double,
                      maxValue: Double) extends ValueNode{
  override def value(position: Vector3): Double = minValue max node.value(position) min maxValue
}

case class ModValue(node: ValueNode,
                    mod: Double) extends ValueNode{
  override def value(position: Vector3): Double = node.value(position) % mod
}


// combine two value nodes
case class AddValues(node1: ValueNode,
                     node2: ValueNode) extends ValueNode{
  override def value(position: Vector3): Double = node1.value(position) + node2.value(position)
}

case class SubtractValues(node1: ValueNode,
                          node2: ValueNode) extends ValueNode{
  override def value(position: Vector3): Double = node1.value(position) - node2.value(position)
}

case class MultiplyValues(node1: ValueNode,
                          node2: ValueNode) extends ValueNode{
  override def value(position: Vector3): Double = node1.value(position) * node2.value(position)
}

case class MixValues(node1: ValueNode,
                     node2: ValueNode,
                     mixValue: ValueNode) extends ValueNode{
  override def value(position: Vector3): Double =  {
