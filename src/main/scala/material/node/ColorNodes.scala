...
// map single value
case class ClampColor(node: ColorNode, minValue: Double, maxValue: Double) extends ColorNode {
  override def value(position: Vector3): RGB =
    node.value(position).map(minValue max _ min maxValue).clamp
}

case class ModColor(node: ColorNode, mod: Double) extends ColorNode {
  override def value(position: Vector3): RGB = node.value(position).map(_ % mod).clamp
}

// mix nodes
case class AddColor(node1: ColorNode, node2: ColorNode) extends ColorNode {
  override def value(position: Vector3): RGB = (node1.value(position) + node2.value(position)).clamp
}

case class SubtractColor(node1: ColorNode, node2: ColorNode) extends ColorNode {
  override def value(position: Vector3): RGB = (node1.value(position) - node2.value(position)).clamp
}

case class MultiplyColor(node1: ColorNode, node2: ColorNode) extends ColorNode {
  override def value(position: Vector3): RGB =
    (node1.value(position) mult node2.value(position)).clamp
}

case class MixColor(node1: ColorNode, node2: ColorNode, mixValue: ValueNode) extends ColorNode {
  override def value(position: Vector3): RGB = {
