...
sealed case class Cuboid(center: Vector3,
  ...
  private val vertices: Seq[Vector3] =  {
    ...
    Seq(Vector3(x,0,0),Vector3(-1*x,0,0),
        ...
        Vector3(0,0,z), Vector3(0,0,-1*z)).map( rotatePoint(_) + center)

  }

  private val unrotated = AABB(center.x-sideLengths.x/2, center.x+sideLengths.x/2,
                               center.y-sideLengths.y/2, center.y+sideLengths.y/2,
                               center.z-sideLengths.z/2, center.z+sideLengths.z/2,
                                material)
  ...
  override lazy val maxZ = vertices.map(_.z).max
    ...
    private def rotateRay(ray: Ray,  center: Vector3 = Vector3.ZERO, rotateInverse : Boolean = false) : Ray= {
    ...
    private def rotatePoint(point: Vector3, center: Vector3 = Vector3.ZERO, rotateInverse : Boolean = false) : Vector3 = {
      val matrix = if(rotateInverse)
        inverseRotMatrix
      else rotMatrix
