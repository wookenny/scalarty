...
object Gradient {
  ...
  def gradient[T](x: Double, y: Double)(implicit function: TwoDimensionalFunction[T],
                                        op: T => Gradientable[T]): (T, T) = {
    ((function.eval(x + EPS, y) - function.eval(x - EPS, y)) / (2 * EPS),
     (function.eval(x, y + EPS) - function.eval(x, y - EPS)) / (2 * EPS))
  }

  def gradient[T](x: Double, y: Double, z: Double)(implicit function: ThreeDimensionalFunction[T],
                                                   op: T => Gradientable[T]): (T, T, T) = {
    implicit def gradFromT(t: T) = op(t)
