...
class VectorSpec extends Specification with ScalaCheck {
  ...
  val negation = forAll { (a: Vector3) =>
    -a == Vector3(-a.x, -a.y, -a.z) && -(-a) == a
  }
  val identity = forAll { (a: Vector3) =>
    +a == a
  }

  val addition = forAll { (p: Vector3, q: Vector3) =>
    q + p == Vector3(p.x + q.x, p.y + q.y, p.z + q.z)
  }
