...
class SamplingValueSpec extends Specification  with ScalaCheck{
  ...
  val failForWrongAdaptiveValues = forAll(Gen.choose(3, 1000)) { full => Try(SamplingValue(full,full-1)).isFailure }

  val readsGeneralValues = forAll(Gen.posNum[Int], Gen.posNum[Int]){ (full,adaptive) =>


    SamplingValue.samplingValueRead.reads(s"$full:$adaptive") == SamplingValue(full max 1 ,if(adaptive > full) adaptive else 0)
  }

  val readInteger = forAll(Gen.posNum[Int]){ x : Int => SamplingValue.samplingValueRead.reads(s"$x") == SamplingValue(x max 1,0)}

  val readIntegerWithColon = forAll(Gen.posNum[Int]){ x : Int => SamplingValue.samplingValueRead.reads(s"$x:") == SamplingValue(x max 1,0)}
